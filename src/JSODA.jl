...
funs = filter(x -> filename(x)==header, ctx.children)
...
#=
mutable struct Ctx{F,D,C<:Common,O<:Opt}
    f::F
    data::D
    neq::Int
    state::Int
...
mutable struct Common
    yh::Vector{Vector{Float64}}
    wm::Vector{Vector{Float64}}
    ewt::Vector{Float64}
    savf::Vector{Float64}
    acor::Vector{Float64}
    ipvt::Vector{Int}
    memory::Vector{Nothing}
    h::Float64
    hu::Float64
    rc::Float64
    tn::Float64
    ...
    pdnorm::Float64
    crate::Float64
    el::NTuple{14,Float64}
    elco::NTuple{13,NTuple{14,Float64}}
    tesco::NTuple{13,NTuple{4,Float64}}
    hold::Float64
    rmax::Float64
    pdest::Float64
    pdlast::Float64
...
function check_opt(ctx::Ctx, opt::Opt)
    mxstp0 = 500
    mord = (0, 12, 5)
    ...
    if ctx.neq <= 0
        println(stderr, "[lsoda] neq = $(ctx.neq) is less than 1")
        return 0
    end
    ...
    if ctx.state == 1 || ctx.state == 3
        for i in 1:ctx.neq
            ...
            atoli = atol[i]
            if rtoli < 0.
                println(stderr, "[lsoda] rtol = $rtoli is less than 0.")
            end
            if atoli < 0.
                println(stderr, "[lsoda] atol = $atoli is less than 0.")
                return 0
            end
        end # end for
    end # end if ( ctx.state == 1 || ctx.state == 3 )

    # default itask is 1
    opt.itask == 0 && (opt.itask = 1)
...
function allocate!(ctx::Ctx)
    nyh = ctx.neq
    lenyh = 1 + max(ctx.opt.mxordn, ctx.opt.mxords)
    ctx.common.yh
end
=#
